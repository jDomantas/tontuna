...
impl<'a> LocationTranslator<'a> {
    ...
    pub(crate) fn to_tontuna<T: ToTontuna>(&mut self, t: T) -> T::TontunaRepr {
        t.to_tontuna(self)
    }
...
pub(crate) trait ToTontuna: Sized {
    type TontunaRepr;

    fn to_tontuna(self, translator: &mut LocationTranslator<'_>) -> Self::TontunaRepr;
}

impl ToTontuna for Range {
    type TontunaRepr = Span;

    fn to_tontuna(self, translator: &mut LocationTranslator<'_>) -> Self::TontunaRepr {
        Span::new(
            translator.to_tontuna(self.start),
            translator.to_tontuna(self.end),
        )
    }
}

impl ToTontuna for Position {
    type TontunaRepr = Pos;
...
mod tests {
    ...
    #[test]
    fn translate_to_tontuna() {
        let source = "012345678\n01234567";
        let mut translator = LocationTranslator::for_source(source);
        for i in 0..10 {
            assert_eq!(Pos::new(i), translator.to_tontuna(Position { line: 0, character: i }));
        }
        for i in 0..=8 {
            assert_eq!(Pos::new(10 + i), translator.to_tontuna(Position { line: 1, character: i }));
        }
        assert_eq!(Pos::new(18), translator.to_tontuna(Position { line: 1, character: 9 }));
        assert_eq!(Pos::new(18), translator.to_tontuna(Position { line: 1, character: 10 }));
        assert_eq!(Pos::new(18), translator.to_tontuna(Position { line: 2, character: 0 }));
        assert_eq!(Pos::new(18), translator.to_tontuna(Position { line: 2, character: 15 }));
        assert_eq!(Pos::new(18), translator.to_tontuna(Position { line: 45, character: 82 }));
    }
}
